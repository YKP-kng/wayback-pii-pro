...
def build_targets(raw: str) -> List[str]:
    ...
    if re.match(r"^https?://", raw, re.I):
        ...
        if not h:
            return []
        # Plain handle, no domain given: check it both as a Twitter/X handle
        # AND as a plain "<handle>.com" domain, so generic names (e.g. "CNN",
        # "nasa") aren't silently assumed to be Twitter-only.
        return [f"x.com/{h}", f"twitter.com/{h}", f"mobile.twitter.com/{h}", f"{h}.com"]
...
if captures:
    ...
    if len(visible) <= 10:
        ...
        st.caption(f"Showing all {max_rows} visible row(s).")
    else:
        max_rows = st.slider("Visible rows", min_value=10, max_value=min(1000, len(visible)), value=min(100, len(visible)), step=10)
    st.subheader("Bulk PII extraction")
    if len(visible) == 0:
        st.info("No rows to process yet.")
        bulk_limit = 0
    elif len(visible) == 1:
        bulk_limit = 1
        st.caption("Only 1 visible row - it will be processed.")
    else:
        bulk_limit = st.number_input("How many visible rows should be processed in bulk?", min_value=1, max_value=min(1000, len(visible)), value=min(50, len(visible)), step=10 if len(visible) >= 10 else 1)
    if bulk_limit and st.button("Run bulk PII and prepare Excel", type="primary"):
