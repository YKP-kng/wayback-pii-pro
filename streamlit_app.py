...
def cdx_search(target, collapse="digest", limit=5000, fields="timestamp,original,mimetype,statuscode,digest,length"):
    params = {"url": target, "matchType": "prefix", "output": "json", "fl": fields, "filter": "statuscode:200", "limit": str(limit)}
    if collapse:
    ...
    for row in data[1:]:
        ...
        obj["target"] = target
        obj["search_mode"] = "direct_capture"
        rows.append(obj)
    return rows

@st.cache_data(ttl=1800, show_spinner=False)
def url_index_search(target, limit=10000):
    # Matches the Wayback "search all archived pages under this URL" / URLs tab behavior.
    params = {"url": target, "matchType": "prefix", "output": "json", "fl": "original,mimetype,timestamp,statuscode,digest,length", "filter": "statuscode:200", "collapse": "urlkey", "limit": str(limit)}
    url = "https://web.archive.org/cdx/search/cdx?" + urllib.parse.urlencode(params)
    data = requests.get(url, headers=HEADERS, timeout=60).json()
    if not isinstance(data, list) or len(data) < 2:
        return []
    ...
    for row in data[1:]:
        obj = dict(zip(header, row))
        obj["target"] = target
        obj["search_mode"] = "url_index_fallback"
        rows.append(obj)
    return rows
...
def search_all(targets, fallback=True):
    ...
    for t in targets:
        try:
            ...
            errors.append({"target": t, "phase": "direct", "error": str(e)})
    seen, direct = set(), []
    for r in rows:
    ...
    meta = {"direct_count": len(direct), "fallback_used": False, "fallback_count": 0}
    if direct or not fallback:
        return direct, errors, meta
    fb = []
    for t in targets:
        try:
            fb.extend(url_index_search(t))
        except Exception as e:
            errors.append({"target": t, "phase": "url_index_fallback", "error": str(e)})
    seen, out = set(), []
    for r in fb:
        k = r.get("original")
    ...
    meta.update({"fallback_used": True, "fallback_count": len(out)})
...
if st.button("Search captures", type="primary"):
    ...
    if not targets:
        ...
        if meta.get("fallback_used"):
            st.info(f"No direct captures were found. Fallback URL-index search found {meta.get('fallback_count', 0):,} archived URLs under this prefix.")
        else:
